...
export default function Experience() {
  return (
    <section
      id="Experience"
      className="p-[20px] md:px-[7.5rem] flex w-full flex-col items-center gap-5 pt-[3.125rem]  pb-[3.125rem] sm:pb-[6.25rem] md:px-[1.875rem] md:gap-[1.5rem]"
      style={{ background: 'rgba(147, 107, 232, 0.05)' }}
    >
      {EXPERIENCES.map((item: any, index: number) => (
        <div className="rounded-xl bg-[#fff]" key={index}>
          <div className="p-[25px] md:p-[2rem]  w-full items-start justify-start flex-col md:flex flex gap-[1.25rem] md:gap-10 relative lt:flex-row lt:w-[56rem]">
            <div className="flex w-[7.125rem] flex-col items-start gap-3 shrink-0">
              <p className=" text-[#7D11F9] text-base not-italic font-semibold leading-6 flex-col sm:flex">{item.label}</p>
            </div>
            <div className="flex-1">
              <h3 className="self-stretch text-[#7D11F9] text-xl not-italic font-semibold">{item.position}</h3>
            ...
            <div className="lt:w-[180px] absolute   right-[2.5rem] lt:sticky lt:flex lt:justify-end">
              <span className="text-[#7D11F9] text-right text-base not-italic font-normal">{item.time}</span>
            </div>
          </div>
        </div>
      ))}
