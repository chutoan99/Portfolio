export default function ProjectInfo({ project }: { project: any }) {
  return (
    <div className="flex flex-col items-center gap-[1rem] lt:gap-[1.125rem]  flex-[1_0_0] self-stretch rounded-[0.75rem_0rem_0rem_0.75rem] ">
      <div className="flex gap-[10px] items-center w-full lt:w-[30rem] flex-col lt:flex-row">
        <h3 className=" self-stretch overflow-hidden text-label text-ellipsis text-xl not-italic font-semibold leading-7">{project.name}:</h3>
        <div className="lt:flex-1 flex justify-start w-full">
          <a target="_blank" rel="noopener noreferrer" href={project.ref} className="text-[#4B5563] text-[1rem] border-b-black border-b border-solid h-[1.3rem]">
            {project.demo}
          </a>
        </div>
      </div>
      ...
      <div className="flex items-center content-center gap-2 self-stretch flex-wrap">
        {project.technologies.map((item: string, index: number) => (
          <span key={index} className=" flex justify-center items-center px-3 lt:px-5 py-1 rounded-xl hover:opacity-50 hover:cursor-pointer hover:transition-[0.3s] bg-[#e5e7eb]">
            {item}
          </span>
        ))}
      </div>
